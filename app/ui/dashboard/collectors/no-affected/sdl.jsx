...
import { ArrowUpDownIcon, MoreHorizontal, Phone, Search, Workflow } from "lucide-react";
...
export default function NoAffectedSDLs({ isLoading: externalLoading }) {
    ...
    useEffect(() => {
        const getSdls = async () => {
            setLoading(true);
            try {
                const response = await fetchData("get", "cafe/stationslavage/get_sdl_non_responsable/", {
                    params: {
                        limit: limit,
                        offset: pointer,
                        ...filterData,
    ...
    const handleExportSDLs = async () => {
        ...
        try {
            const initResponse = await fetchData("get", `cafe/stationslavage/get_sdl_non_responsable/`, {
                params: { limit: 1 },
            });
