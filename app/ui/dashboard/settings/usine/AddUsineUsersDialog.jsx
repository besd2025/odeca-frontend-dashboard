...
export default function AddUsersUsineDialog() {
        ...
        async function loadInitialData() {
            try {
                const [provData, socData] = await Promise.all([
                    ...
                    fetchData("get", `cafe/usine_deparchage/get_all_usine/`, { params: { offset: 0, limit: 12 } })
                ]);
                ...
                setSocieteOptions(socData?.map(s => ({
                    value: s.usine_code,
                    label: s.usine_name
                })) || []);
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        const FormData = {
            ...
            usine_code: formData.usine_code
        }

        try {
            const response = await fetchData("post", `cafe/registration_udp/`, {
                body: FormData,
            });
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="flex items-center gap-2 font-normal text-sm">
                    ...
                    Nouveau responsable d'usine
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[700px] bg-sidebar">
                <DialogHeader>
                    <DialogTitle>Ajouter un responsable d'usine</DialogTitle>
                    <DialogDescription>
                        Remplissez les informations ci-dessous pour créer un nouveau responsable d'usine.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit}>
                    <div className="custom-scrollbar h-[60vh] lg:max-h-[500px] overflow-y-auto px-2 pb-3 mt-4 space-y-6">
                        ...
                        {/* affiliation section */}
                        <div className="space-y-4">
                            <h5 className="text-lg font-medium text-primary">Usine</h5>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label>Usine</Label>
                                    <select
